...
export default function TodayPage() {
  ...
  const items = [
    ...transports.filter((item) => item.departureDate === active).map((item) => ({ id: `t-${item.id}`, href: `/trips/${id}/transports/${item.id}/edit`, time: item.departureTime, type: "移動", title: `${item.departurePlace} → ${item.arrivalPlace}`, detail: `${TRANSPORT_MODE_LABELS[item.mode]} ${item.serviceName} ${[item.carNumber, item.seatNumber, item.departurePlatform].filter(Boolean).join("・")}`, place: item.arrivalPlace })),
    ...schedules.filter((item) => item.date === active).map((item) => ({ id: `s-${item.id}`, href: `/trips/${id}/schedule/${item.id}/edit`, time: item.startTime, type: "予定", title: item.title, detail: item.place, place: item.place })),
    ...reservations.filter((item) => item.startAt.slice(0, 10) === active).map((item) => ({ id: `r-${item.id}`, href: `/trips/${id}/reservations/${item.id}/edit`, time: item.startAt.slice(11, 16), type: "予約", title: item.name, detail: item.type, place: "" })),
  ...
  return <main className="min-h-screen bg-stone-100 px-4 py-5 text-stone-900"><div className="mx-auto max-w-3xl">
    ...
    <section className="mt-6"><h2 className="text-lg font-bold">1日の流れ</h2>{items.length ? <ol className="mt-3 space-y-2">{items.map((item, itemIndex) => <li key={item.id}><a href={item.href} aria-label={`${item.type}「${item.title}」の詳細を開く`} className={`grid min-h-20 grid-cols-[3.5rem_1fr_auto] gap-3 rounded-xl p-4 active:scale-[0.99] focus-visible:outline-2 focus-visible:outline-teal-700 ${itemIndex === highlight ? "bg-white ring-2 ring-teal-700" : itemIndex < (highlight >= 0 ? highlight : items.length) ? "bg-white/50 text-stone-400" : "bg-white"}`}><time className="text-sm tabular-nums">{item.time || "--:--"}</time><div><p className="text-xs font-bold">{itemIndex === highlight && "次："}{item.type}</p><p className="mt-0.5 break-words font-bold">{item.title}</p>{item.detail && <p className="mt-1 break-words text-sm opacity-75">{item.detail}</p>}</div><span className="self-center text-stone-300" aria-hidden="true">›</span></a></li>)}</ol> : <p className="mt-3 rounded-xl bg-white p-5 text-sm text-stone-500">この日の予定はまだありません。</p>}</section>
    <div className="mt-6 grid grid-cols-2 gap-3"><button type="button" onClick={() => dispatchEvent(new Event(OPEN_QUICK_RECORD_EVENT))} className="min-h-14 rounded-xl bg-teal-700 px-3 font-bold text-white">写真・思い出を残す</button><button type="button" onClick={() => dispatchEvent(new Event(OPEN_QUICK_ADD_EVENT))} className="min-h-14 rounded-xl bg-white px-3 font-bold text-stone-700">旅の情報を追加</button></div>
