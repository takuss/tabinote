...
export default function NewRecordPage() {
  const { id } = useParams<{ id: string }>(); const q = useSearchParams(); const { trips, isLoaded } = useTrips(); const trip = trips.find((item) => item.id === id); const prefill = { amount: q.get("amount") ?? "", date: q.get("date") ?? undefined, time: q.get("time") ?? undefined, expenseCategory: q.get("category") === "交通" ? "交通" as const : undefined, type: "移動" as const, title: "交通の費用", memo: q.get("memo") ?? "" };
  return <Shell>{!isLoaded ? <p className="text-sm text-stone-500">旅行を読み込んでいます…</p> : !trip ? <Missing /> : <><div className="border-b border-stone-300 pb-4"><Link href={`/trips/${trip.id}`} className="text-sm font-medium text-teal-800 hover:underline">← 旅行詳細に戻る</Link><h1 className="mt-4 text-xl font-bold sm:text-2xl">旅の記録を追加する</h1><p className="mt-1 text-sm text-stone-500">{trip.title}</p></div><RecordForm trip={trip} prefill={prefill} /></>}</Shell>;
}
