...
function SectionHeading({ id, title, href, linkLabel = "記録を追加" }: { id: string; title: string; href: string; linkLabel?: string }) { return <div className="flex items-center justify-between border-b border-stone-300 pb-3"><h2 id={id} className="text-lg font-bold">{title}</h2><Link href={href} className="inline-flex min-h-11 items-center text-sm font-bold text-teal-800 hover:underline">{linkLabel}</Link></div>; }
