...
export default function BackupManager() {
  ...
  return <section aria-labelledby="backup-heading" className="mt-10 border-t border-stone-200 pt-6"><details>
    ...
    <p className="mt-1 text-xs leading-5 text-stone-400">表紙写真は現在のJSONバックアップ対象外です。</p>
    ...
    {preview && <div className="mt-5 rounded-xl bg-amber-50 p-4 text-sm text-amber-950"><p className="font-bold">復元内容を確認してください</p><p className="mt-2 leading-6">旅行 {preview.trips}件・予定 {preview.schedules}件・予約 {preview.reservations}件・記録 {preview.records}件（支出 {preview.expenses}件）</p><p className="mt-2 font-bold">現在のデータはすべて置き換えられます。この操作は取り消せません。</p><div className="mt-4 flex gap-3"><button type="button" onClick={() => { setPreview(null); if (inputRef.current) inputRef.current.value = ""; }} className={`${secondaryButtonClass} flex-1`}>キャンセル</button><button type="button" onClick={restore} disabled={busy} className={`${primaryButtonClass} flex-[2]`}>{busy ? "復元中…" : "すべて置き換えて復元"}</button></div></div>}
  </details></section>;
}
