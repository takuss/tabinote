...
export default function QuickReservationForm({ trip, onClose }: { trip: Trip; onClose: () => void }) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const values: ReservationValues = {
      type: String(data.get("quickReservationType") ?? "") as ReservationValues["type"],
  ...
  return <form onSubmit={handleSubmit} noValidate className="mt-4 rounded-lg border border-teal-200 bg-teal-50/60 p-4 sm:p-5">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2">
      <Field label="予約種別" error={errors.type} htmlFor="quickReservationType"><select id="quickReservationType" name="quickReservationType" defaultValue="宿泊" aria-invalid={Boolean(errors.type)} className={inputClass}>{RESERVATION_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></Field>
